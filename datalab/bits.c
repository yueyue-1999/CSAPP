...
int conditional(int x, int y, int z) {
  int num = ((!x)<<31)>>31; //若x=0,num=1111...1111; 若x!=0,num=0000...0000 
  return ((~num)&y) + (num&z); //y&0000...0000得到0，y&1111...1111得到y
}
/* 
...
unsigned floatScale2(unsigned uf) {
  int sign = (uf>>31)&0x00000001;
  int exp = (uf>>23)&0x000000ff;
  int frac = uf&0x007fffff;
  if (exp == 0xff) return uf;
  else if (exp == 0x00) frac = frac<<1;
  else exp++;
...
int floatFloat2Int(unsigned uf) {
  ...
  int exp = (uf>>23)&0x000000ff;
  int frac = uf&0x007fffff;
  if ((exp-127)>23) return 0x80000000u;
  else if ((exp-127)<0) return 0;
  else //正常情况
  {
    frac = (frac|0x00800000)>>(23-(exp-127));
...
unsigned floatPower2(int x) {
  if (x > 128) return 0x7f800000;//特殊值
  else if (x < -(126+23)) return 0; //非规格化，全0
  else if (x >= (-126+23) && x < -126) return 1<<(23+126+x); //非规格化，后面还有位(exp部分表示的是2^(-126))
  else
  {
    x = x + 127;
    return x<<23;
